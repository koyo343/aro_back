...
async def search_user(
    name: Optional[str] = None, age: Optional[int] = None, sex: Optional[int] = None, prefecture: Optional[str] = None, city: Optional[str] = None, 
    githubid: Optional[str] = None, favolang: Optional[str] = None, wantlang: Optional[str] = None, db: AsyncSession = Depends(get_db)
) -> List[user_schema.User]:
    return await user_crud.search_user(
        db=db, name=name, age=age, sex=sex, prefecture=prefecture, city=city, 
        githubid=githubid, favolang=favolang, wantlang=wantlang
    )
